...
How to add or change the context the AI relies on here. For authoring AIDD skills, agents, rules, and templates, see: <https://github.com/ai-driven-dev/framework/blob/main/CONTRIBUTING.md>.
...
- Memory captures decisions and domain language; coding rules that must always apply live in `.cursorrules` / `.clinerules`
- Do not hand-edit the `<aidd_project_memory>` block in `AGENTS.md`; change files under `memory/` and re-run sync
